...
Let's create an `account.sql`, `entry.sql`, and `transfer.sql` files inside db/query with the following queries:
...
The above command will generate the following go files inside /db/sqlc:
